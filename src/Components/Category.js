...
const Category = () => {
  ...
  const { isLoading, data } = useGetMovieByCategoryQuery({ path, page: 1 });
  if (isLoading) {
  ...
  return (
    <>
      ...
      <div className="flex gap-10 mx-auto w-full items-center mb-10 p-5 justify-center">
        {data?.page !== 1 && <button className="bg-blue-800 text-white px-4 py-2 rounded-2xl font-bold">Prv</button>}<p>{data?.page}</p><button onClick={() => nav(`/moviepage/ ${path}/ ${data?.page + 1}`)} className="bg-blue-800 text-white px-4 py-2 rounded-2xl font-bold">Next</button>
      </div>
    </>
  )
}
